...
async function compile (input, output, template) {
    let mdContent = await readFileAsync(input, 'utf8')
    let filename = extname(input)
    ...
    if (output && info.title && info.created) {
        // info.htmlFile = join(output, `${info.created}-${simpleTitle(info.title)}.html`)
        let f = input.replace(filename, '')
        info.htmlFile = join(output, `${info.created}-${f}.html`)
    }
    ...
    if (output) {
        if(template) {
            await writeFileAsync(info.htmlFile, render(await readFileAsync(template, 'utf8'), {
                html: md.render(body),
                title: info.title,
                filename: input
            }))
